#!/usr/bin/env node
/** Valida fluxo Memed com idExterno fresco (evita hang de re-bind no mesmo atendimento). */
...
process.env.MEMED_TEST_ID_EXTERNO =
  process.env.MEMED_TEST_ID_EXTERNO ||
  `${process.env.ATENDIMENTO_ID}-validacao-${Date.now()}`;
...
execSync(
  'npx playwright test e2e/memed-minimal-payload-fresh-id.spec.ts --project=memed-minimal-payload-fresh-chromium --reporter=line',
  {
    stdio: 'inherit',
    env: process.env,
    cwd: root,
