 ...
 *
 * - Pergunta sim/não em has_warning_signs
 ...
 *
 * Uso local (default):
 *   node mdoctor-backend/scripts/patch-typebot-sinais-alerta.js
 *
 * Publicar no Typebot (opcional):
 *   TYPEBOT_API_TOKEN=*** node mdoctor-backend/scripts/patch-typebot-sinais-alerta.js --publish
 */
...
function isAlreadyPatched(typebot) {
  const groupSinais = typebot.groups.find((g) => g.id === GROUP_SINAIS_ID);
  if (!groupSinais) return false;
  const textBlock = groupSinais.blocks.find((b) => b.id === TEXT_BLOCK_ID);
  const text = textBlock?.content?.richText?.[0]?.children?.[0]?.text || '';
  const choice = groupSinais.blocks.find((b) => b.id === CHOICE_BLOCK_ID);
  return (
    text === QUESTION &&
    choice?.options?.variableId === VAR_HAS_WARNING_SIGNS &&
    choice?.items?.length === 2 &&
    !groupSinais.blocks.some((b) => b.id === OLD_CONDITION_BLOCK_ID)
  );
}
...
function patchLocalFile(filePath) {
  ...
  if (!fs.existsSync(resolved)) {
    return { file: resolved, skipped: true, reason: 'file_not_found' };
  }

  const before = JSON.parse(fs.readFileSync(resolved, 'utf8'));
  if (isAlreadyPatched(before)) {
    return { file: resolved, skipped: true, reason: 'already_patched' };
  }
...
async function publishToTypebot(typebot) {
  ...
  const patchRes = await fetch(`https://app.typebot.io/api/v1/typebots/${TYPEBOT_ID}`, {
    ...
    headers,
    body: JSON.stringify({
      typebot: { version: typebot.version, groups: typebot.groups, edges: typebot.edges },
      overwrite: true
    })
  });
  if (!patchRes.ok) throw new Error('PATCH falhou: ' + (await patchRes.text()).slice(0, 400));

  const publishRes = await fetch(`https://app.typebot.io/api/v1/typebots/${TYPEBOT_ID}/publish`, {
    method: 'POST',
    headers
  });
  if (!publishRes.ok) throw new Error('PUBLISH falhou: ' + (await publishRes.text()).slice(0, 400));

  return { patchStatus: patchRes.status, publishStatus: publishRes.status };
}
...
async function main() {
  ...
  const results = files.map(patchLocalFile);
  const productionFile = path.resolve(DEFAULT_FILES[0]);
  const productionResult = results.find((r) => r.file === productionFile);
  const productionData = productionResult?.skipped
    ? JSON.parse(fs.readFileSync(productionFile, 'utf8'))
    : JSON.parse(fs.readFileSync(productionFile, 'utf8'));

  let publishResult = null;
  if (publish) {
    publishResult = await publishToTypebot(productionData);
  }
